...
no_sub_x = 2;  %number of sub-divisions of elements in x-direction INCREASE
no_sub_y = 2;  %number of sub-divisions of elements in y-direction
%Rfocus = 3e-3;
focus = [0,0,12e-2];  %(m) Fixed focus for array (x,y,z). Vector with three elements. 
...
% Scatterer is mat filed from MUST code
...
for b = 1:nbeams
    ...
    xdc_focus(Tx, -1*ones(17535,1), ScatLoc);
    xdc_dynamic_focus(Rx, -1, 0, 0);
