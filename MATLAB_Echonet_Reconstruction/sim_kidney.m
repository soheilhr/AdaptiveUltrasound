% Kidney phantom imaging
% Routine field.m initializes the field system ; modify to point to the
% directory holding the Field II code and m files. 
% make_scatterers.m is called to make the file for the scatterers in the
% phantom
% kidney_cut.bmp holds the scatterer map of the kidney
% Script sim_kidney.m is called; here the field simulation is performed and
% the data is stored in RF files, one for each RF line that is done. 
% the data for the scatterers are read from the file pht_data.mat so that
% the procedure can be started again or run for a number of work stations. 
% The data must then be subsequently processed to do the polar to
% rectangular mapping to yield the image. This is done by make_polar which
% creates an image with a dynamic range of 50 dB. 
